...
1. Identify the account alias, instrument, side, margin mode, position side, order type, price, and size from the user's request. Use the account environment detected by the Runtime; do not ask the user to classify the API key or invent missing trading intent.
